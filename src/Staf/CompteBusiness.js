...
const handlesubmit=e=>{
   ...
   // console.log(data)
    if(data.nom===""|| data.phonegaalguimoney==="" ||
        ...
        data.nom.match(/^ *$/)!== null||data.description===""){
    erreur()
    return;}
    else{
      let   formdata=new FormData()
        formdata.append('nom',data.nom)
        formdata.append('description_business',data.description)
        ...
        axiosInstance
        .post('staff/createbusiness/',formdata)
        .then(res=>{
           //console.log(res.data)
            history.push(`/qrcodebusiness/${res.data.id}/${res.data.nom}`)
        })
        .catch(()=>{
          badresponse()
        })
    ...
    return (
        ...
        {load?
        <>
       <h1 className='container mb-4'>Creation de  Compte Business GaalguiMoney</h1>
    ...
    <p className='centerbtn'> <label  className="w3-container"><b>Numero de telephone lie au compte GaalguiMoney</b></label></p>
     ...
     <p className='centerbtn'> <label  className="w3-container"><b>Contact du client
