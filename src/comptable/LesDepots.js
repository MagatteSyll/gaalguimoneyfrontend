...
function  LesDepots() {
 ...
 useEffect(()=>{
  ...
  const getdepot=()=>{
     axios
    .get('http://127.0.0.1:8000/api/staff/getlesdepots/',
    {headers:{
     'Authorization': `JWT ${localStorage.getItem('__jmdf__')}`
    }})
   // axiosInstance
    //.get('staff/getlesdepots/')
    .then(res=>{
      setdepot(res.data.results)
  ...
  <IonCol size='10' className='container'>
   <TableContainer component={Paper}>
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        ...
        {depot.map(d=>
        <TableRow key={d.id}>
       <TableCell component="th" scope="row">{d.depositaire.prenom} {d.depositaire.nom}</TableCell>
        <TableCell align="right">{d.somme}</TableCell>
        <TableCell align="right">{new Date(d.created).toLocaleDateString('en-GB',
                     {hour: '2-digit', minute:'2-digit'})}</TableCell>
       <TableCell align="right">{d.employe.point_acces.region.region},{d.employe.point_acces.adress}</TableCell>
       <TableCell align='right'> <Switch  
       checked={!d.relever}
       onChange={()=>handlerelever(d.id)}
...
<ReactPaginate
  ...
  :<IonCol size='10'>
  <h3 className='centerbtn'> Aucun nouveau depot</h3> </IonCol>}
  </IonGrid>
  </div>
  :null}
   </div>
