...
function  LesRetraits() {
 ...
 useEffect(()=>{
  ...
  const getretrait=()=>{
  axios
  .get('http://127.0.0.1:8000/api/staff/getlesretraits/',
    {headers:{
  'Authorization': `JWT ${localStorage.getItem('__jmdf__')}`
    }})
    //axiosInstance
    //.get('staff/getlesretraits/')
    .then(res=>{
...
const handlerelever=id=>{
  axiosInstance
  .post('staff/releveretrait/',{id:id})
  .then(res=>{
    getretrait()
  })
}
...
const handledisplay=(data)=>{
  ...
  axios
  .get(`http://127.0.0.1:8000/api/staff/getlesretraits/?page=${page}`,
    {headers:{
  'Authorization': `JWT ${localStorage.getItem('__jmdf__')}`
    }})
 // axiosInstance
 // .get(`staff/getlesretraits/?page=${page}`)
  .then(res=>{
    setretrait(res.data.results)
      setnext(res.data.next)
      setprevious(res.data.previous)
 ...
 return(
  ...
  <h4 className='centerbtn'>Les retraits</h4>
   <TableContainer component={Paper}>
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        ...
        {retrait.map(r=>
        <TableRow key={r.id}>
       <TableCell component="th" scope="row">{r.beneficiaire.prenom} {r.beneficiaire.nom}</TableCell>
...
<ReactPaginate
  ...
  :
  <h3 className='centerbtn'> Aucun nouveau retrait</h3>}
  </IonGrid>
  </div>
  :null}
   </div>
