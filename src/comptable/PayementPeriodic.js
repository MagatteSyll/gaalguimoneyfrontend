...
function PayementPeriodic(){
 ...
 useEffect(()=>{
  ...
  const getperiod=()=>{
    axiosInstance
    .get('staff/getlespayementsperiodic/')
    .then(res=>{
      setperiod(res.data)
      setload(true)
...
const handlerelever=id=>{
  axiosInstance
  .post('staff/relevepayementperiodic/',{id:id})
  .then(res=>{
    getperiod()
  })
}
 ...
 {load?
  ...
  <h4 className='centerbtn'>Les payements periodiques</h4>
  ...
  :<IonCol size='10'>
  <h3 className='centerbtn'> Aucun nouveau payement periodique </h3> </IonCol>}
  </IonRow>
  </IonGrid>
  </>
  :null}
...
export default PayementPeriodic